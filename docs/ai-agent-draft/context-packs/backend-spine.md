...
Guardrails
- Do not add database or migrations during a pure endpoint/module cleanup.
...
- If scoring prototype changed:
  - `(cd prototypes/jvm-scoring && mvn test)`
  - `python3 -B scripts/prototype_contract_parity.py`
- If Spring preview prototype is still intended to match:
...
- Does the endpoint behavior match `docs/api-shape.md` or deliberately defer
  from it?
