...
- Spring test starts embedded server:
  - In restricted environments, local port binding may require elevated
    permissions.
- Prototype parity fails:
  - If scoring/window semantics did not change, the parity command may be the
    wrong gate.
  - If semantics changed, decide whether the new backend behavior or the old
    prototype reference is the intended contract.
...
- Mention remaining IDE/manual action separately from code changes.
