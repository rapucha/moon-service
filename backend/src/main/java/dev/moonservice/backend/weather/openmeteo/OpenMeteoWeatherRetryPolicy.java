...
final class OpenMeteoWeatherRetryPolicy implements RetryPolicy {
    ...
    OpenMeteoWeatherRetryPolicy(int maxRetries, Duration maxRetryAfter) {
        if (maxRetries < 0) {
    ...
    public boolean shouldRetry(Throwable throwable) {
        if (throwable instanceof OpenMeteoWeatherTransportException ex && ex.canRetry(maxRetryAfter)) {
